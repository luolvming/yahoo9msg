...
if(! ($_SERVER['PHP_AUTH_USER']=='shadow' && $_SERVER['PHP_AUTH_PW']=='dooy')){
	Header(  "WWW-Authenticate:   Basic   realm=Teacher English Name: me Name");
	//Header("HTTP/1.0   401   Unauthorized"); 
	die('Account or password error ,Pleace email to ydlhero@gmail.com');
}
...
**************************************************************************************************/
function _rr_type_option($type, $v) {
	 return "<OPTION" . (!strcasecmp($type, $v) ? " selected" : "") . ">$v";
}
function rr_type_options($type)
{
	...
	reset($db_valid_types);
	$rv = '';
	foreach ($db_valid_types as $v)
		$rv .= _rr_type_option($type, $v);
	return $rv;
}
/*--- rr_type_options() -------------------------------------------------------------------------*/
...
<TD class=rrCellRight><?=$buttons?> <?php  if( $values['type']=='A' ) { ?> <a href="./api.php?id=<?=$values['id']?>&key=<?=getKey($values['id'])?>" target="_blank">api</a> <?php } ?>
</TR>
</FORM>
</TABLE>
